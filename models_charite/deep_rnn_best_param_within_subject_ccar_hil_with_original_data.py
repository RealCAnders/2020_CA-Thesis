...
class TemporalSplit(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        print(inputs)
        print(inputs.shape)
        return tf.split(inputs, 20, axis=2)
    ...
    def compute_mask(self, inputs, mask=None):
        ...
        if mask is None:
            return None
        return tf.split(mask, 20, axis=2)
...
for title in ['epoched_intrplt_kx_data_combined',
	...
	for k in range(1, 11):
        ...
        model = build_model_after_deeprnn(optimizer='sgd', learning_rate=0.001, clip_vals=True, n_timewin=20, in_shape=(250, 20))
        ...
        znorm_noise = normalize_z(hil_extracted_ccar_hfSEP)
        znorm_hfsep = normalize_z(hil_extracted_ccar_noise)

        znorm_noise = np.concatenate((normalize_z(noise_k00x[:8,:,:]), znorm_noise), axis=0) ## TODO: Continue checking here!!!
        znorm_hfsep = np.concatenate((normalize_z(hfsep_k00x[:8,:,:]), znorm_hfsep), axis=0)
        ...
        history = model.fit(x=X_train, y=y_train, epochs=3, batch_size=32, validation_data=[X_eval, y_eval]) 

        TODO: Add here the stuff from the deep_rnn_bnest_para_with_subject_ccar_hil-file
