...
for title in ['epoched_intrplt_kx_data_combined',
    ...
    for k in range(1, 11): 
        ...
        ccar_filt_hfSEP_1 = np.tensordot(a_ccar[:,1], hfsep_k00x[:8,:,:], axes=(0, 0))
        ccar_filt_noise_1 = np.tensordot(a_ccar[:,1], noise_k00x[:8,:,:], axes=(0, 0))
        ccar_filt_hfSEP_2 = np.tensordot(a_ccar[:,2], hfsep_k00x[:8,:,:], axes=(0, 0))
        ccar_filt_noise_2 = np.tensordot(a_ccar[:,2], noise_k00x[:8,:,:], axes=(0, 0))
        ...
        # ToDo: ReRun this!
        znorm_noise = normalize_z(hil_extracted_ccar_noise)
        znorm_hfsep = normalize_z(hil_extracted_ccar_hfSEP)
        ...
        model.save('/media/christoph/Volume/Masterthesis/new_model_trained_and_saved/ccar_hil/%s/deep_rnn_w_%s_lr_%f_clip_%s_k00%d' % (title, 'sgd', 0.001, 'True', k))
        np.save('/media/christoph/Volume/Masterthesis/histories_new_model_trained_and_saved/ccar_hil/%s/deep_rnn_w_%s_lr_%f_clip_%s_k00%d' % (title, 'sgd', 0.001, 'True', k), history.history)
        ...
        np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place_confusion_matrices/ccar_hil/deep_rnn_w_sgd_lr_0001_clip_True_k00%d_tn_%d_fp_%d_fn_%d_tp_%d_confusion_matrix' % (k, tn, fp, fn, tp), confusion_matrix_for_model)

        metrics = metrics_for_conf_mat(tn, fp, fn, tp)
        np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place_confusion_matrices/ccar_hil/deep_rnn_w_sgd_lr_0001_clip_True_k00%d_tn_%d_fp_%d_fn_%d_tp_%d_metrics' % (k, tn, fp, fn, tp), metrics)