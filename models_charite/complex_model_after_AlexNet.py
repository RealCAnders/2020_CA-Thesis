...
model.add(layers.Conv2D(16 * i, (11, 11), input_shape=(185, 198, 4), activation='relu', padding='same'))
...
model.add(layers.Conv2D(16 * i, (k, k), activation=l, padding='same'))
model.add(layers.Conv2D(32 * i, (5, 5), activation='relu', padding='same'))
model.add(layers.MaxPooling2D((2, 2)))
model.add(layers.Conv2D(16 * i, (5, 5), activation=l, padding='same'))
model.add(layers.Conv2D(32 * i, (k, k), activation='relu', padding='same'))
model.add(layers.GlobalAveragePooling2D())
model.add(layers.Dropout(0.1))
model.add(layers.Flatten())
model.add(layers.Dense(64 * i, activation=l, kernel_constraint=tf.keras.constraints.MaxNorm(3)))
model.add(layers.Dropout(0.2))
...
model.save('/media/christoph/Volume/Masterthesis/models_trained_and_saved_test_place/complex_2d_cnn_after_alexnet_%d_%f_%d_%s_%d' % (i, j, k, l, m))
np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place/complex_2d_cnn_after_alexnet_%d_%f_%d_%s_%d' % (i, j, k, l, m), history.history)