...
#-#-#
#-#-#    # some model definition and training
#-#-#    model = create_alex_net(X_train.shape[0])
#-#-#
#-#-#    history = model.fit(x=X_train, y=y_train, epochs=25, batch_size=32, validation_data=[X_eval, y_eval]) 
#-#-#
#-#-#    model.save('/media/christoph/Volume/Masterthesis/alex_net/models/%d/alex_net_on_%s' % (idx, run_title))
#-#-#    np.save('/media/christoph/Volume/Masterthesis/alex_net/history/%d/alex_net_on_%s' % (idx, run_title), history.history)
#-#-#
#-#-#    predictions = np.abs(np.rint(model.predict(x=X_test)))
#-#-#    confusion_matrix_for_model = confusion_matrix(y_test, predictions)
#-#-#    tn, fp, fn, tp = confusion_matrix_for_model.ravel()
#-#-#    np.save('/media/christoph/Volume/Masterthesis/alex_net/%d/alex_net_on_%s_confusion_matrix' % (idx, run_title), confusion_matrix_for_model)
#-#-#
#-#-#    metrics = metrics_for_conf_mat(tn, fp, fn, tp)
#-#-#    np.save('/media/christoph/Volume/Masterthesis/alex_net/%d/alex_net_on_%s_metrics' % (idx, run_title), metrics)
