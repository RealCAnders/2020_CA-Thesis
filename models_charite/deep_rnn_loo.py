...
for id_to_leave_out in range(9, 11):

	model = build_model_after_deeprnn(optimizer='sgd', learning_rate=0.001, clip_vals=True)

	history = model.fit(x=tf.data.Dataset.from_generator(lambda: loo_data_generator_train(id_to_leave_out), (tf.float32, tf.uint8), ([250, 8], [None])).batch(32), epochs=25) 

	model.save('/media/christoph/Volume/Masterthesis/models_trained_and_saved_test_place/deep_rnn_w_sgd_lr_0001_clip_True_loo_k00%d_left_out' % id_to_leave_out)
	np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place/deep_rnn_w_sgd_lr_0001_clip_True_loo_k00%d_left_out' % id_to_leave_out, history.history)

	predictions = np.abs(np.rint(model.predict(x=tf.data.Dataset.from_generator(lambda: loo_data_generator_test(id_to_leave_out), (tf.float32), ([250, 8])).batch(32))))
	...
	np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place_confusion_matrices/deep_rnn_w_sgd_lr_0001_clip_True_loo_k00%d_left_out_tn_%d_fp_%d_fn_%d_tp_%d_confusion_matrix' % (id_to_leave_out, tn, fp, fn, tp), confusion_matrix_for_model)
