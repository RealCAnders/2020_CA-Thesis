...
def load_data_from_modality_for_all_but_to_leave_out(modality_id, identifier_to_leave_out):
  ...
  for i in range(starter_id, end_id_so_far_loaded):
    if i is not identifier_to_leave_out:
      new_to_add_data = load('/media/christoph/Volume/Masterthesis/DEAP_TSC/data/s%d/%s.npy' % (i, modality_id), allow_pickle=True)[:32,:,:]
...
def load_data_from_modality_for_one_to_leave_out(modality_id, identifier_to_leave_out):
  ...
  for i in range(starter_id, end_id_so_far_loaded):
    if i is identifier_to_leave_out:
      all_subjects_data = np.concatenate((all_subjects_data, load('/media/christoph/Volume/Masterthesis/DEAP_TSC/data/s%d/%s.npy' % (i, modality_id), allow_pickle=True)[:32,:,:]), axis=2)
...
runs_to_do = [
#  ['epoched_filt_under_50_kx', 'CSP', 2],
#  ['epoched_filt_under_50_kx', 'CSP', 3],
  ['epoched_filt_under_50_kx', 'CSP_hil', 1],
  ['epoched_filt_under_50_kx', 'CSP_hil', 2],
  ['epoched_filt_under_50_kx', 'CSP_hil', 3],
#  ['epoched_filt_under_50_kx', 'CCAR_hil', 1],
#  ['epoched_filt_over_100_kx', 'CSP', 2],
#  ['epoched_filt_over_100_kx', 'CSP', 3],
  ['epoched_filt_over_100_kx', 'CSP_hil', 1],
  ['epoched_filt_over_100_kx', 'CSP_hil', 2],
  ['epoched_filt_over_100_kx', 'CSP_hil', 3]
]
