...
print('### ### ### ### ### ###')
print('Computing variance_ratios for outlier removed data of k00%d' % (identifier + 1))
...
epchd_14_25_ms_intrplt_filt_500_900 = meet.epochEEG(intrplt_filt_500_900_kx, triggers_kx_data_combined, ccar_win)
a, b, s = meet.spatfilt.CCAvReg(epchd_14_25_ms_intrplt_filt_500_900[:8,:,:])

ccar_filt_epoched_kx_data_combined_hfsep = np.tensordot(a[:,0], epchd_14_25_ms_intrplt_filt_500_900[:8,:,:], axes=(0, 0))
...
for idx, hfsep_data, noise_data, level in [(0, epchd_14_25_ms_intrplt_kx_data_combined, epchd_30_60_ms_intrplt_kx_data_combined_baseline, 'data_combined'), 
	...
	for i in averaging_points:    
		...
		# Swap from (chan, time_in_sample, trial) to (trial, chan, time_in_sample)
		...
		var_rat_slid = np.std(std_over_time_cp5_minus_fz_pos_slid) / np.std(std_over_time_cp5_minus_fz_neg_slid)
		var_rat_jump = np.std(std_over_time_cp5_minus_fz_pos_jump) / np.std(std_over_time_cp5_minus_fz_neg_jump)
