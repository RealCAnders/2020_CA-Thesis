...
for title in ['epoched_intrplt_kx_data_combined',
    ...
    for k in range(1, 11): 
        ...
        model.save('/media/christoph/Volume/Masterthesis/new_model_trained_and_saved/ccar/%s/deep_rnn_w_%s_lr_%f_clip_%s_k00%d' % (title, 'sgd', 0.001, 'True', k))
        np.save('/media/christoph/Volume/Masterthesis/histories_new_model_trained_and_saved/ccar/%s/deep_rnn_w_%s_lr_%f_clip_%s_k00%d' % (title, 'sgd', 0.001, 'True', k), history.history)
        ...
        np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place_confusion_matrices/ccar/%s/deep_rnn_w_sgd_lr_0001_clip_True_k00%d_tn_%d_fp_%d_fn_%d_tp_%d_confusion_matrix' % (title, k, tn, fp, fn, tp), confusion_matrix_for_model)

        metrics = metrics_for_conf_mat(tn, fp, fn, tp)
        np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place_confusion_matrices/ccar/%s/deep_rnn_w_sgd_lr_0001_clip_True_k00%d_tn_%d_fp_%d_fn_%d_tp_%d_metrics' % (title, k, tn, fp, fn, tp), metrics)