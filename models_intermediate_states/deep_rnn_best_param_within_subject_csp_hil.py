...
for title in ['epoched_intrplt_filt_over_400_kx',
    ...
    for k in range(1, 11): 
        ...
        model = build_model_after_deeprnn(optimizer='sgd', learning_rate=0.001, clip_vals=True, n_timewin=12, in_shape=(250, 12))
        ...
        hfSEP_CSP_1 = np.tensordot(csp_filters[1].T, hfsep_k00x[:8,:,:], axes=(0 ,0))
        noise_CSP_1 = np.tensordot(csp_filters[1].T, noise_k00x[:8,:,:], axes=(0 ,0))
        hfSEP_CSP_2 = np.tensordot(csp_filters[2].T, hfsep_k00x[:8,:,:], axes=(0 ,0))
        noise_CSP_2 = np.tensordot(csp_filters[2].T, noise_k00x[:8,:,:], axes=(0 ,0))
        ...
        znorm_noise = normalize_z(hil_extracted_CSP_noise)
        znorm_hfsep = normalize_z(hil_extracted_CSP_hfSEP)
        ...
        model.save('/media/christoph/Volume/Masterthesis/new_model_trained_and_saved/csp_hil/%s/deep_rnn_w_%s_lr_%f_clip_%s_k00%d' % (title, 'sgd', 0.001, 'True', k))
        np.save('/media/christoph/Volume/Masterthesis/histories_new_model_trained_and_saved/csp_hil/%s/deep_rnn_w_%s_lr_%f_clip_%s_k00%d' % (title, 'sgd', 0.001, 'True', k), history.history)
        ...
        np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place_confusion_matrices/csp_hil/%s/deep_rnn_w_sgd_lr_0001_clip_True_k00%d_tn_%d_fp_%d_fn_%d_tp_%d_confusion_matrix' % (title, k, tn, fp, fn, tp), confusion_matrix_for_model)

        metrics = metrics_for_conf_mat(tn, fp, fn, tp)
        np.save('/media/christoph/Volume/Masterthesis/histories_of_models_trained_and_saved_test_place_confusion_matrices/csp_hil/%s/deep_rnn_w_sgd_lr_0001_clip_True_k00%d_tn_%d_fp_%d_fn_%d_tp_%d_metrics' % (title, k, tn, fp, fn, tp), metrics)
