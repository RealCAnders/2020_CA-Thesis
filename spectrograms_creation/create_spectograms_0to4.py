...
# K003 und K006 brauchen beide noch ab kx_data_intrplt... out_rej, ebenfalls K004. Die anderen komplett. Die Krux: How to spektogramm operation?
...
for i in [2, 3, 0, 1, 4]:
	...
	for file_path in data_to_be_analyzed_in_cp5_min_fz:
		...
		for idx, sec in enumerate(one_sec_samples_of_ccar_filt_k003.T):
			...
			#plt.colorbar()
			spectograms.append(np.abs(tf_mean_p_interp[:1500, :2500]))
			spectograms.append(np.abs(tf_mean_p_interp[:1500, 2500:5000]))
			spectograms.append(np.abs(tf_mean_p_interp[:1500, 5000:7500]))
			spectograms.append(np.abs(tf_mean_p_interp[:1500, 7500:]))
			if idx == 0:
				f_range = f[:2501]
				t_range = t[:1501]
