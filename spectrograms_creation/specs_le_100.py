...
one_sec_win = [0, 10000]                                                                                                                                                             
one_sec_samples_of_ccar_filt_k003 = meet.epochEEG(intrplt_filt_under_100_kx[0] - intrplt_filt_under_100_kx[5], np.arange(0, 1300000, 10000), one_sec_win)
...
for idx, sec in enumerate(one_sec_samples_of_ccar_filt_k003.T):  
  ...
  tmp.append(np.asarray([tmp0[i,:] / tmp0[i,:].mean() for i in range(tmp0.shape[0])]))  
  tmp.append(np.asarray([tmp1[i,:] / tmp0[i,:].mean() for i in range(tmp1.shape[0])]))  
  tmp.append(np.asarray([tmp2[i,:] / tmp0[i,:].mean() for i in range(tmp2.shape[0])]))  
  tmp.append(np.asarray([tmp3[i,:] / tmp0[i,:].mean() for i in range(tmp3.shape[0])]))  
  tmp = np.asarray(tmp)  
...
plt.pcolormesh(t_range, f_range, 20 * np.log10(mean_spectrogram), cmap=plt.cm.plasma, vmin=0, vmax=10)                                                                                                        
bar = plt.colorbar()                                                                                                                                                                
...
extension_avg = '3_s%dspec%s500avg'
...
plt.pcolormesh(t_range, f_range, 20 * np.log10(single_trial_spec), cmap=plt.cm.plasma, vmin=0, vmax=10)
bar = plt.colorbar()                                                                                                                                                                
bar.set_label('amplitude', fontsize=20)
bar.ax.tick_params(labelsize=20)
...
plt.ylim([0, 1400])                                                                                                                                                               
plt.tick_params(labelsize=20)
extension_st = '2_s%dspec%ssingletrial'
