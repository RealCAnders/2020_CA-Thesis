...
def create_DenseConvNet_for_2d_adjusted(input_len, amount_of_channels):
  ...
  print('Received shape for construction of :%d and %d' % (input_len, amount_of_channels))

  # Define the respective channels for the electrodes signals
  model = keras.models.Sequential([  
    keras.layers.Conv2D(filters=25, kernel_size=(amount_of_channels,1), input_shape=(amount_of_channels,input_len,input_len)),  
  ...
  model.compile(optimizer=tf.keras.optimizers.SGD(0.00001, momentum=0.9, clipvalue=5.0),  
...
def create_shallow_conv_net_for_2d_adjusted(input_len, amount_of_channels):
  ...
  # Define the respective channels for the electrodes signals
  model = keras.models.Sequential([  
      keras.layers.Conv2D(filters=40, kernel_size=(1,10), input_shape=(amount_of_channels,input_len,input_len)),  
...
def create_mc_cnn_for_2d_adjusted(input_len, amount_of_channels):
  ...
  for channel_id in range(amount_of_channels):
    input_channel = tf.keras.Input((input_len, input_len), name=('Channel-%d' % channel_id))
...
def EEGNet(nb_classes, Chans = 64, Samples = 128, 
    ...
    input1   = keras.Input(shape = (Chans, Samples, Samples))
    ...
    eeg_net.compile(optimizer=tf.keras.optimizers.SGD(0.00001, momentum=0.9, clipvalue=5.0),  
      loss=tf.keras.losses.BinaryCrossentropy(), 
      metrics=METRICS)
...
datasets_ucr = [
#  ['/home/christoph/Downloads/Thesis_Last_Minute_TSC/FingerMovements/FingerMovements_%s.arff', "b'left'", 1],
...
#  ['/home/christoph/Downloads/Thesis_Last_Minute_TSC/MotorImagery/MotorImagery_%s.arff', "b'tongue'"],
  ['/home/christoph/Downloads/Thesis_Last_Minute_TSC/SelfRegulationSCP1/SelfRegulationSCP1_%s.arff', "b'negativity", 3],
  ['/home/christoph/Downloads/Thesis_Last_Minute_TSC/SelfRegulationSCP2/SelfRegulationSCP2_%s.arff', "b'negativity", 3],
]
...
for idx, ucr_tuple in enumerate(datasets_ucr):
  ...
  divider_level = ucr_tuple[2]
  if divider_level > 1:
    X_train = convolve1d(X_train, weights=np.ones(divider_level) / divider_level, axis=1)[:,int(divider_level / 2):-int(divider_level / 2):divider_level]
    X_test = convolve1d(X_test, weights=np.ones(divider_level) / divider_level, axis=1)[:,int(divider_level / 2):-int(divider_level / 2):divider_level]
  ...
  for data, labels, run_title in workload:
    ...
    X_train = np.asarray([rp.fit_transform(np.swapaxes(x_dat, 0, 1)) for x_dat in X_train])
    X_test = np.asarray([rp.fit_transform(np.swapaxes(x_dat, 0, 1)) for x_dat in np.swapaxes(np.swapaxes(np.swapaxes(workload_test[ctr][0], 2, 1), 1, 0), 1, 2)])
    X_eval = np.asarray([rp.fit_transform(np.swapaxes(x_dat, 0, 1)) for x_dat in X_eval])
    ...
    if idx <= -1:
      ...
      for model_tuple in models:
        ...
        model.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/2d_tsc_rec_plots_adjusted_learning_rate_to_be_small/%s/models/%s_on_%s' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')))
        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/2d_tsc_rec_plots_adjusted_learning_rate_to_be_small/%s/histories/%s_on_%s' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')), history.history)
        ...
        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/2d_tsc_rec_plots_adjusted_learning_rate_to_be_small/%s/metrics/%s_on_%s_confusion_matrix' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')), confusion_matrix_for_model)
      
        metrics = metrics_for_conf_mat(tn, fp, fn, tp)
        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/2d_tsc_rec_plots_adjusted_learning_rate_to_be_small/%s/metrics/%s_on_%s_metrics' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')), metrics)
