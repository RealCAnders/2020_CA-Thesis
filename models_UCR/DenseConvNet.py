...
def load_data_as_still_sorted_hfsep_noise_data_then_labels_from_one_subject(X_train, y_train, title_of_run):
  ...
  csp_filters, csp_eigenvals = meet.spatfilt.CSP(X_train[:,:,y_train==1].mean(2), X_train[:,:,y_train==0].mean(2))
  ...
  # ToDo: Continue here!
  pos_labels = np.ones(len(X_train[:,:,y_train==1]), dtype=np.int8)
  neg_labels = np.zeros(len(X_train[:,:,y_train==0]), dtype=np.int8)
...
for idx, ucr_tuple in enumerate(datasets_ucr):
  ...
  for data, labels, run_title in workload:
    ...
    if idx <= 1:
      ...
      models = [
        [create_DenseConvNet_for_2d_adjusted(), 'dense_conv_net'],
        [create_shallow_conv_net_for_2d_adjusted(), 'shallow_conv_net'],
        [EEGNet(), 'eeg_net'],
        [create_mc_cnn_for_2d_adjusted(), 'mc_cnn']
      ]
      ...
      for model_tuple in models:
        ...
        if model_tuple[1] in 'mc_cnn':  
          ...
          history = model.fit(x=X_train, y=y_train, epochs=2, batch_size=32, validation_data=[X_eval, y_eval])

        model.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/models/%s_on_%s' % (model_tuple[1], run_title.replace('/', '-')))
        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/histories/%s_on_%s' % (model_tuple[1], run_title.replace('/', '-')), history.history)
        ...
        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/metrics/%s_on_%s_confusion_matrix' % (model_tuple[1], run_title.replace('/', '-')), confusion_matrix_for_model)
      
        metrics = metrics_for_conf_mat(tn, fp, fn, tp)
        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/metrics/%s_on_%s_metrics' % (model_tuple[1], run_title.replace('/', '-')), metrics)

    else:
      # For the datasets FaceDetection and FingerMovements the models had to be adjusted
      models = [
        [create_DenseConvNet_for_2d_original(), 'dense_conv_net'],
        [create_shallow_conv_net_for_2d_original(), 'shallow_conv_net'],
        [EEGNet(), 'eeg_net'],
        [create_mc_cnn_for_2d_original(), 'mc_cnn']
      ]
    ...
    # Early-Stopping criterion for now.
    if ctr==2:
      break
