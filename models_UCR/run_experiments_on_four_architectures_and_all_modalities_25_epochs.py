...
def load_data_from_ucr_set(data_path, class_0_identifier, train_test_id):
  # Load data
  path = data_path
  train_dat = arff.loadarff(path % train_test_id)
  train_dat_pd = pd.DataFrame(train_dat[0])
  train_dat_np = np.asarray(train_dat_pd) 

  y_train = np.asarray([0 if x.decode('utf-8') in class_0_identifier else 1 for x in train_dat_np[:,1]])
  ...
  # return data in format of (channel x epoch_length x number of epochs)
...
def load_data_as_still_sorted_hfsep_noise_data_then_labels_from_one_subject(X_train, y_train, title_of_run):
  ...
  csp_filters, csp_eigenvals = meet.spatfilt.CSP(X_train[:,:,y_train==1].mean(2), X_train[:,:,y_train==0].mean(2), center=True)
  pos_CSP_0 = np.tensordot(csp_filters[0].T, X_train[:,:,y_train==1], axes=(0 ,0))
  neg_CSP_0 = np.tensordot(csp_filters[0].T, X_train[:,:,y_train==0], axes=(0 ,0))
  pos_CSP_1 = np.tensordot(csp_filters[1].T, X_train[:,:,y_train==1], axes=(0 ,0))
  neg_CSP_1 = np.tensordot(csp_filters[1].T, X_train[:,:,y_train==0], axes=(0 ,0))
  pos_CSP_2 = np.tensordot(csp_filters[2].T, X_train[:,:,y_train==1], axes=(0 ,0))
  neg_CSP_2 = np.tensordot(csp_filters[2].T, X_train[:,:,y_train==0], axes=(0 ,0))

  # Compute CCAr
  ccar_title = title_of_run + '_CCAr'
  a_ccar, b_ccar, s_ccar = meet.spatfilt.CCAvReg(X_train[:,:,y_train==1])
  ccar_filt_pos_0 = np.tensordot(a_ccar[:,0], X_train[:,:,y_train==1], axes=(0, 0))
  ccar_filt_neg_0 = np.tensordot(a_ccar[:,0], X_train[:,:,y_train==0], axes=(0, 0))
  ccar_filt_pos_1 = np.tensordot(a_ccar[:,1], X_train[:,:,y_train==1], axes=(0, 0))
  ccar_filt_neg_1 = np.tensordot(a_ccar[:,1], X_train[:,:,y_train==0], axes=(0, 0))
  ccar_filt_pos_2 = np.tensordot(a_ccar[:,2], X_train[:,:,y_train==1], axes=(0, 0))
  ccar_filt_neg_2 = np.tensordot(a_ccar[:,2], X_train[:,:,y_train==0], axes=(0, 0))

  # Compute bCSTP
  # s_bcstp_eigenvals, t_bcstp_eigenvals, W_bcstp, V_bcstp = bCSTP(X_train[:,:,y_train==0], X_train[:,:,y_train==0], num_iter=15, t_keep=3, s_keep=3)
  ...
  hil_extracted_pos_CSP_0 = calculate_hil_features(pos_CSP_0)
  hil_extracted_neg_CSP_0 = calculate_hil_features(neg_CSP_0)
  hil_extracted_pos_CSP_1 = calculate_hil_features(pos_CSP_1)
  hil_extracted_neg_CSP_1 = calculate_hil_features(neg_CSP_1)
  hil_extracted_pos_CSP_2 = calculate_hil_features(pos_CSP_2)
  hil_extracted_neg_CSP_2 = calculate_hil_features(neg_CSP_2)
  hil_extracted_CSP_hfSEP = np.concatenate((hil_extracted_pos_CSP_0, hil_extracted_pos_CSP_1, hil_extracted_pos_CSP_2), axis=0)
  hil_extracted_CSP_noise = np.concatenate((hil_extracted_neg_CSP_0, hil_extracted_neg_CSP_1, hil_extracted_neg_CSP_2), axis=0)

  hil_ccar_title = title_of_run + '_CCAR_hil'
  hil_extracted_ccar_filt_pos_0 = calculate_hil_features(ccar_filt_pos_0)
  hil_extracted_ccar_filt_neg_0 = calculate_hil_features(ccar_filt_neg_0)
  hil_extracted_ccar_filt_pos_1 = calculate_hil_features(ccar_filt_pos_1)
  hil_extracted_ccar_filt_neg_1 = calculate_hil_features(ccar_filt_neg_1)
  hil_extracted_ccar_filt_pos_2 = calculate_hil_features(ccar_filt_pos_2)
  hil_extracted_ccar_filt_neg_2 = calculate_hil_features(ccar_filt_neg_2)
  hil_extracted_ccar_hfSEP = np.concatenate((hil_extracted_ccar_filt_pos_0, hil_extracted_ccar_filt_pos_1, hil_extracted_ccar_filt_pos_2), axis=0)
  hil_extracted_ccar_noise = np.concatenate((hil_extracted_ccar_filt_neg_0, hil_extracted_ccar_filt_neg_1, hil_extracted_ccar_filt_neg_2), axis=0)

  # ToDo: Continue here!
  pos_labels = np.ones(X_train[:,:,y_train==1].shape[2], dtype=np.int8)
  neg_labels = np.zeros(X_train[:,:,y_train==0].shape[2], dtype=np.int8)

  # return the datasets in epoch, channel, time_in_channel - fashion
  return [
    [np.concatenate((X_train[:,:,y_train==1], X_train[:,:,y_train==0]), axis=-1), np.concatenate((pos_labels, neg_labels), axis=0), raw_title],
#   [np.concatenate((hfSEP.reshape(-1, hfSEP.shape[-1]), noise.reshape(-1, noise.shape[-1])), axis=1), np.concatenate((pos_labels, neg_labels), axis=0), title_of_run + '_all_channels_flattened'],// I have no idea why, but this singular error still occurs
    [np.concatenate((np.stack((pos_CSP_0, pos_CSP_1, pos_CSP_2), axis=0), np.stack((neg_CSP_0, neg_CSP_1, neg_CSP_2), axis=0)), axis=2), np.concatenate((pos_labels, neg_labels), axis=0), csp_title],
    [np.concatenate((np.stack((ccar_filt_pos_0, ccar_filt_pos_1, ccar_filt_pos_2), axis=0), np.stack((ccar_filt_neg_0, ccar_filt_neg_1, ccar_filt_neg_2), axis=0)), axis=2), np.concatenate((pos_labels, neg_labels), axis=0), ccar_title],
    [np.concatenate((hil_extracted_CSP_hfSEP, hil_extracted_CSP_noise), axis=2), np.concatenate((pos_labels, neg_labels), axis=0), hil_csp_title],
    [np.concatenate((hil_extracted_ccar_hfSEP, hil_extracted_ccar_noise), axis=2), np.concatenate((pos_labels, neg_labels), axis=0), hil_ccar_title]
  ]


def load_data_as_still_sorted_hfsep_noise_data_then_labels_from_one_subject_with_filters(X_train, y_train, title_of_run, csp_filters=None, a_ccar=None):
  ### 01_ToDo: Modify all the 11 base-modalities through: [SSD Y/N] (load respective modifiers therefore)
  # Still will remain open. Only thing to do here however: Load non-epoched data, compute SSD, epoch data
  raw_title = title_of_run + '_raw'
  print('Shape of data: ')
...
def load_data_concatenated_of_different_modalities(X_train, y_train, title_of_run):
  ### 01_ToDo: Modify all the 11 base-modalities through: [SSD Y/N] (load respective modifiers therefore)
  # Still will remain open. Only thing to do here however: Load non-epoched data, compute SSD, epoch data
  raw_title = title_of_run + '_raw'
  print('Shape of data: ')
  ...
  ### , np.concatenate((pos_labels, neg_labels), axis=0), raw_title],

  h_a = X_train[:,:,y_train==1]
  h_c = np.stack((pos_CSP_0, pos_CSP_1, pos_CSP_2), axis=0)
  h_d = np.stack((ccar_filt_pos_0, ccar_filt_pos_1, ccar_filt_pos_2), axis=0)
  h_e = hil_extracted_CSP_hfSEP
  h_f = hil_extracted_ccar_hfSEP

  h_a = h_a.reshape(h_a.shape[-1], -1)
  h_c = h_c.reshape(h_c.shape[-1], -1)
  h_d = h_d.reshape(h_d.shape[-1], -1)
  h_e = h_e.reshape(h_e.shape[-1], -1)
  h_f = h_f.reshape(h_f.shape[-1], -1)

  n_a = X_train[:,:,y_train==0]
  n_c = np.stack((neg_CSP_0, neg_CSP_1, neg_CSP_2), axis=0)
  n_d = np.stack((ccar_filt_neg_0, ccar_filt_neg_1, ccar_filt_neg_2), axis=0)
  n_e = hil_extracted_CSP_noise
  n_f = hil_extracted_ccar_noise

  n_a = n_a.reshape(n_a.shape[-1], -1)
  n_c = n_c.reshape(n_c.shape[-1], -1)
  n_d = n_d.reshape(n_d.shape[-1], -1)
  n_e = n_e.reshape(n_e.shape[-1], -1)
  n_f = n_f.reshape(n_f.shape[-1], -1)

  pos_conc = np.concatenate((h_a, h_c, h_d, h_e, h_f), axis=-1)
  noise_conc = np.concatenate((n_a, n_c, n_d, n_e, n_f), axis=-1)

  return [np.concatenate((pos_conc, noise_conc), axis=0), np.concatenate((pos_labels, neg_labels), axis=0), raw_title]
...
######
# define early-stopping to be used for all models if using early stopping. used so far only for alex_net
...
### AlexNet
# adjustments done:
#   pool size to 4 instead of 3
#   Insertion of Dense layer of 512 and dropout 0.4 as well as others increased to 0.7
def create_alex_net_1d(input_len, learning_rate=0.001):
  ...
  model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate, momentum=0.9, clipvalue=5.0),  
...
### ELM
def create_ELM(dimensionality=1000):
  return meet.elm.ClassELM(dimensionality)
...
#-#-#
#-#-#        model.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/models/%s_on_%s' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')))
#-#-#        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/histories/%s_on_%s' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')), history.history)
      #-#-#
#-#-#        if model_tuple[1] in 'mc_cnn':
...
#-#-#        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/metrics/%s_on_%s_confusion_matrix' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')), confusion_matrix_for_model)
      #-#-#
#-#-#        metrics = metrics_for_conf_mat(tn, fp, fn, tp)
#-#-#        np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/metrics/%s_on_%s_metrics' % (model_tuple[1], model_tuple[1], run_title.replace('/', '-')), metrics)
#-#-#
#-#-#    else:
#-#-#      # For the datasets FaceDetection and FingerMovements the models had to be adjusted
#-#-#      models = [
...
# spectral filtering prior to other things!
...
#-#fp=100, fs=110, s_rate=1000
#-#fp=100, fs=90, s_rate=1000
#-#fp=400, fs=360, s_rate=1000
#-#fp=[500, 900], fs=[450, 1000], s_rate=1000
...
### Include spectral filtering here, if possible
#-#  for i in range(5):
#-#
#-#    if i == 0:
#-#      X_train = 
...
#-#      plt
  ...
  workload, csp_filter, ccar_filter = load_data_as_still_sorted_hfsep_noise_data_then_labels_from_one_subject_with_filters(X_train, y_train, ucr_tuple[0].split('/')[-1].split('_')[0])
  workload_test, csp_filter, ccar_filter = load_data_as_still_sorted_hfsep_noise_data_then_labels_from_one_subject_with_filters(X_test, y_test, ucr_tuple[0].split('/')[-1].split('_')[0], csp_filter, ccar_filter)

  ctr = 0
  for data, labels, run_title in workload:
    print('run_title: %s' % run_title)
    print(data.shape)
#   change which model to train
#    elm = create_ELM()
#ELM-Part:    ### Shuffle and split data // .T is required to switch back to shape of (trial x feature)
#ELM-Part:    X_train, y_train = shuffle(data.T, labels, random_state=rand_stat)
#ELM-Part:    print(X_train.shape)
#ELM-Part:
#ELM-Part:    # results in shape: (chan x sample_time x trial x 1):e.g.:(7, 200, 1152, 1)
#ELM-Part:    X_train = np.swapaxes(X_train, 1, 2)
#ELM-Part:    X_train = X_train.reshape(X_train.shape[0], -1)
#ELM-Part:    X_test = np.swapaxes(np.swapaxes(workload_test[ctr][0], 1, 2), 0, 1)
#ELM-Part:    X_test = X_test.reshape(X_test.shape[0], -1)

    # part for alex_net
    #alex_net = create_alex_net_1d(data.shape[0] * data.shape[1])
    alex_net = create_alex_net_1d(data.shape[0] * data.shape[1], learning_rate=0.0001)
    shuffled_data, shuffled_labels = shuffle(data.T, labels, random_state=rand_stat)
    X_train, X_eval, y_train, y_eval = train_test_split(shuffled_data, shuffled_labels, test_size=0.33, random_state=rand_stat)
    print(X_train.shape)

    # results in shape: (chan x sample_time x trial x 1):e.g.:(7, 200, 1152, 1)
    X_train = np.expand_dims(meet.iir.butterworth(np.swapaxes(X_train, 1, 2), fp=100, fs=110, s_rate=256), axis=-1)
    X_test = np.swapaxes(np.expand_dims(meet.iir.butterworth(np.swapaxes(workload_test[ctr][0], 1, 2), fp=100, fs=110, s_rate=256), axis=-1), 0, 1)
    X_eval = np.expand_dims(meet.iir.butterworth(np.swapaxes(X_eval, 1, 2), fp=100, fs=110, s_rate=256), axis=-1)
...
#-#ELM-Part:    elm.cv(X_train, y_train, folds=5)
#-#ELM-Part:    if elm.istrained:
#-#ELM-Part:#      print('Succesfully trained the ELM. Now work on gaining insights and getting performance metrics.')
#-#ELM-Part:#      print('Performing classification on the test-data now, after optimizing the randomly initialized ELM. Result:')
#-#ELM-Part:      confusion_matrix_after_random_initialization = meet.elm.get_conf_matrix(y_test, elm.classify(X_test))
#-#ELM-Part:#      print(confusion_matrix_after_random_initialization)
#-#ELM-Part:      with open('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/models/%s_on_%s' % ('ELM', 'ELM', run_title.replace('/', '-')), 'wb') as target_file: 
#-#ELM-Part:          pickle.dump(elm, target_file)
    
#-#ELM-Part:    np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/metrics/%s_on_%s_confusion_matrix' % ('ELM', 'ELM', run_title.replace('/', '-')), confusion_matrix_after_random_initialization)

#-#ELM-Part:    tn, fp, fn, tp = confusion_matrix_after_random_initialization.ravel()
#-#ELM-Part:    metrics = metrics_for_conf_mat(tn, fp, fn, tp)
#-#ELM-Part:    np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/metrics/%s_on_%s_metrics' % ('ELM', 'ELM', run_title.replace('/', '-')), metrics)
    
#    history = alex_net.fit(x=X_train, y=y_train, epochs=25, batch_size=32, validation_data=[X_eval, y_eval])
    history = alex_net.fit(x=X_train, y=y_train, epochs=500, batch_size=32, validation_data=[X_eval, y_eval], callbacks=[callback])

    alex_net.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/models/%s_on_%s' % ('alex_net', 'alex_net', run_title.replace('/', '-')))
    np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/histories/%s_on_%s' % ('alex_net', 'alex_net', run_title.replace('/', '-')), history.history)

    predictions = np.abs(np.rint(alex_net.predict(x=X_test)))

    confusion_matrix_for_model = confusion_matrix(y_test, predictions)
    tn, fp, fn, tp = confusion_matrix_for_model.ravel()
    np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/metrics/%s_on_%s_confusion_matrix' % ('alex_net', 'alex_net', run_title.replace('/', '-')), confusion_matrix_for_model)

    metrics = metrics_for_conf_mat(tn, fp, fn, tp)

    print('#### #### #### #### #### #### #### #### ####')
    print('#### #### #### MCC is: %f #### #### ####' % metrics[-1])
    print('#### #### #### #### #### #### #### #### ####')

    np.save('/media/christoph/Volume/Masterthesis/UCR_TSC/scripts_and_results/%s/metrics/%s_on_%s_metrics' % ('alex_net', 'alex_net', run_title.replace('/', '-')), metrics)
