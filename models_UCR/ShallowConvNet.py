...
def create_shallow_conv_net_for_2d(input_len, amount_of_channels):
...
for data, labels, run_title in workload:
  ...
  print('shape: %d, %d' % (X_train.shape[2], X_train.shape[1]))
  # some model definition and training
  model = create_shallow_conv_net_for_2d(X_train.shape[2], X_train.shape[1])
#  history = model.fit(x=[np.squeeze(x, axis=1) for x in np.split(X_train, indices_or_sections=X_train.shape[1], axis=1)], y=y_train, epochs=25, batch_size=32, validation_data=[[np.squeeze(x, axis=1) for x in np.split(X_eval, indices_or_sections=X_eval.shape[1], axis=1)], y_eval]) 
  ...
  model.save('/media/christoph/Volume/Masterthesis/multi-channel-training/shallow_conv_net/models/%d/shallow_conv_net_on_%s' % (idx, run_title.replace('/', '-')))
  np.save('/media/christoph/Volume/Masterthesis/multi-channel-training/shallow_conv_net/history/%d/shallow_conv_net_on_%s' % (idx, run_title.replace('/', '-')), history.history)
  ...
  np.save('/media/christoph/Volume/Masterthesis/multi-channel-training/shallow_conv_net/%d/shallow_conv_net_on_%s_confusion_matrix' % (idx, run_title.replace('/', '-')), confusion_matrix_for_model)

  metrics = metrics_for_conf_mat(tn, fp, fn, tp)
  np.save('/media/christoph/Volume/Masterthesis/multi-channel-training/shallow_conv_net/%d/shallow_conv_net_on_%s_metrics' % (idx, run_title.replace('/', '-')), metrics)
